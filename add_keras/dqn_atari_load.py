...
dqn = DQNAgent(model=model, nb_actions=nb_actions, policy=policy, memory=memory,
               processor=processor, nb_steps_warmup=512, gamma=.99, target_model_update=10000,
               train_interval=4, delta_clip=1., batch_size=512)
dqn.compile(Adam(lr=.00005), metrics=['mae'])

if args.mode == 'train':
    if args.weights:
        checkpoint_path = "./weights_filename/checkpoint"
        checkpoint_dir = os.path.dirname(checkpoint_path)
        latest = tf.train.latest_checkpoint(checkpoint_dir)
        print(latest)
        model.load_weights(latest)
        weights_filename = './weights/dqn_{}_weights.h5f'.format(args.env_name)
        checkpoint_weights_filename = './weights_filename1/dqn_' + args.env_name + '_weights_{step}.h5f'
        log_filename = 'dqn_{}_log.json'.format(args.env_name)

        callbacks = [ModelIntervalCheckpoint(checkpoint_weights_filename, interval=250000)]
        callbacks += [FileLogger(log_filename, interval=100)]
        dqn.fit(env, callbacks=callbacks, nb_steps=20000000, log_interval=10000, nb_max_episode_steps=500)
